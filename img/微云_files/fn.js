        ...
        if(data[i].pid == num){
            for (var z=0 ;z<data.length;z++){
                if(data[z].pid == data[i].id ){
                    html += '<p _id="'+ data[i].id +'"><em class="jia"></em><span></span>'+ data[i].name +'</p>';
                    off = false;
                    break;
                }
            }
            if(off){
                html +='<p _id="'+ data[i].id +'"><em class="wu"></em><span></span>'+ data[i].name +'</p>'
            }
        }
    }
    $('.tips .content').html(html);
    $('.tips .content').on('click','p',function (e) {
        alert(1)
        var _this = $(e.target);
        if (e.target.className == 'jia' ){
            ...
            for (var i=data.length-1;i>0;i--){
                ...
                if(data[i].pid == _id){
                    for (var z=0 ;z<data.length;z++){
                        if(data[z].pid == data[i].id ){
                            $(_this.parent()).after('<p style="padding-left: '+ pad +'px " _id="'+ data[i].id +'"><em class="jia"></em><span></span>'+ data[i].name +'</p>');
                            off = false;
                            break;
                        }
                    }
                    if(off){
                        $(_this.parent()).after('<p style="padding-left: '+ pad +'px " _id="'+ data[i].id +'"><em class="wu"></em><span></span>'+ data[i].name +'</p>');
                    }
                }
            }
